...
ALLOWED_ROOT_DIRS = {".axiom", ".github", "bulk", "data", "programs", "tests", "ec"}
ALLOWED_ROOT_FILES = {
    ".gitignore",
    "CLAUDE.md",
    "README.md",
...
def test_only_ecuador_namespace_present() -> None:
    """Tanzania mainland scope: the only jurisdiction directory is ec/."""
    names = {d.name for d in jurisdiction_dirs()}
    assert names <= {"ec"}, f"unexpected jurisdiction dirs: {names - {'tz'}}"


def test_tz_content_buckets_exist() -> None:
    for marker in ("statutes", "regulations", "policies"):
        assert (ROOT / "ec" / marker).is_dir(), f"missing ec/{marker}"
...
def test_oracle_index_is_tz_scoped() -> None:
    payload = json.loads((ROOT / "data/oracles/oracle-index.json").read_text())
    assert payload["jurisdiction"] == "ec"


def test_source_map_is_tz_scoped() -> None:
    payload = json.loads(
